...
// Package client provide a simple wrapper/structure for initializing HTTP client
// and interact with Percona Platform.
package client
...
func WithLogFullRequest() Option {
	return func(c *Client) {
		...
		c.restyClient.SetTransport(
			logger.HTTP(
				tr,
				"perconaPlatformClient",
				logger.LogFullRequest(),
			),
		)
	}
}
...
// PlatformError is a model of an error response from Percona Platform.
...
func checkForError(resp *resty.Response, err error) error {
	...
	if resp.IsError() {
		if e, ok := resp.Error().(*PlatformError); ok {
			return e
		}
